...
const recipes = ["Борщ червоний", "Стейк", "Хінкалі", "Паста", "Піца", "Суші"];
...
function drawWheel() {
    ...
    recipes.forEach((recipe, i) => {
        ...
        wheel.fillStyle = "#333";
        wheel.font = "16px 'Poppins', sans-serif";
...
function showResult() {
    const selectedIndex = Math.floor(((360 - angle + 30) % 360) / (360 / recipes.length));
    const selectedRecipe = recipes[selectedIndex];
    const resultElement = document.getElementById("result");
    resultElement.innerHTML = `Вам випало: <a href="${selectedRecipe}.html">${selectedRecipe}</a>!`;
    resultElement.style.color = colors[selectedIndex % colors.length];
    resultElement.classList.add('animate');
    setTimeout(() => resultElement.classList.remove('animate'), 2000);
}
...
document.getElementById('recipe-form').addEventListener('submit', function (e) {
    ...
    if (name && message) {
        ...
        newComment.classList.add('recipe-comment');
        newComment.innerHTML = `<p><strong>${name}:</strong> ${message}</p>`;
        document.getElementById('suggested-recipes').appendChild(newComment);
